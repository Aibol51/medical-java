...
public class MemberAuthServiceImpl implements MemberAuthService {
    ...
    private AppAuthLoginRespVO createTokenAfterLoginSuccess(MemberUserDO user, String mobile,
        ...
        // 构建返回结果
        log.info("accessTokenRespDTO: {}", accessTokenRespDTO);
    ...
    @Override
    public AppAuthLoginRespVO smsRegister(AppAuthRegisterReqVO registerReqVO) {
        // 校验验证码
        System.out.print(registerReqVO);
        String userIp = getClientIP();
        // 获得获得注册用户
        MemberUserDO user = userService.registerUser(registerReqVO);
        System.out.print(user);
        Assert.notNull(user, "获取用户失败，结果为空");
        // 创建 Token 令牌，记录登录日志
        return createTokenAfterLoginSuccess(user, user.getMobile(), LoginLogTypeEnum.REGISTER_SMS, null);
    }
