...
public class MemberUserServiceImpl implements MemberUserService {
    ...
    public MemberUserDO registerUser(AppAuthRegisterReqVO registerReqVO) {
        return registerUser(registerReqVO.getMobile(), registerReqVO.getNickname(), registerReqVO.getPassword(), registerReqVO.getCode());
    }

    private MemberUserDO registerUser(String mobile, String nickname, String password, String code) {
        System.out.print(mobile);
        // 校验手机号是否存在
        MemberUserDO existingUser = memberUserMapper.selectByMobile(mobile);
        if (existingUser != null) {
            throw exception(AUTH_MOBILE_USED); // You'll need to define this error code
        }

        smsCodeApi.useSmsCode(new SmsCodeUseReqDTO().setMobile(mobile)
                .setCode(code)
                .setScene(SmsSceneEnum.MEMBER_REGISTER.getScene())
